...
async function generateTidbits() {
  ...
  if (isWindows) tidbits.push(`I see it's working pretty well on your Windows device.`);

  // CPU Cores
  const cores = navigator.hardwareConcurrency || 'unknown';
  tidbits.push(`Mhm. You're reading this on a ${cores > 8 ? 'powerful' : 'decent'} machine. ${cores} cores is ${cores > 12 ? 'quite a handful' : 'enough'}.`);
  ...
  tidbits.push(`I noticed ${isFullscreen ? ' you like to give your apps room to breathe' : 'you prefer a compact view'}—your browser window is ${isFullscreen ? 'almost as big as your screen' : 'not maximized'}.`);
  ...
  tidbits.push(`Lastly, you're safe in ${location.city}, ${location.region}.`)
